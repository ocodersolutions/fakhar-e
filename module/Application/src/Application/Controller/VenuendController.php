...
use Zend\Json\Json;
class VenuendController extends BaseActionController
{
    ...
    public function savestylevenuecheckedAction(){
        if ($this->getRequest()->isPost()){
            $styleid = $this->params()->fromPost('styleId');
            $venueid = $this->params()->fromPost('list_venueid');
        }
        $oSavestyle = $this->getServiceLocator()->get('VenueStyleTable');
            
        $nor = [];
        foreach ($venueid as $key ) {
             $data =array(
                'style_id'=>$styleid,
                'venue_id'=>$key
            );
