...
class OutfitClosesetTable extends BasicTableAdapter {

    protected $table = 'OutfitCloseset';
    ...
    public function getOutfitsProducts($conditions = array(), $searchKey = null, $remdomone = null, $filterarg = null,$userId = null) {
        ...
        $select = $sql->select(array('op' => 'outfits_products'))
                      ->join(array("Ol" => 'OutfitCloseset'), new Expression("(Ol.outfeedid = op.outfit_id)"),array(), 'left');
...
//                 } else 
    ...
    public function saveOutfitToCloset($iOutfitID, $iUserID)
    {
    ...
    public function unSaveEntireOutfit($iOutfitID, $iUserID)
    {
        ...
        foreach ($aOutfitArticles as $articles) {
            $delete = new Delete();
            $delete->from('ArticleCloseset')->where("feeddataid = '{$articles['id']}' and userId = {$iUserID}");
