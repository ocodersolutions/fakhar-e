...
class MyclosetController extends BaseActionController {
    ...
    public function indexAction() {
        ...
        $paginator = $oFeedData->getFeedData($aPostParams, true, $userId);
        ...
        if (isset($aGetParams['outfit']) && $aGetParams['outfit']) {
            ...
            $params = array(
                ...
                'join' =>  array(
                    array(
                        'table' => 'outfits_products',
                        'on' => 'outfits_products.outfit_id = OutfitCloseset.outfeedid',
                        'field' => array(),
                        'type' => 'INNER',
                    )
            ...
            if(count($outfits) > 0) {
                foreach ($outfits as $value) {
                    $outfit_details[$value['outfeedid']] = $oFeedMapping->getOutfitsMapping(array('outfit_id' => $value['outfeedid']), true);
                }
            }
    ...
    public function getfeedsAction() {
        ...
        if ($request->isXmlHttpRequest()) {
            ...
            $paginator = $oFeedData->getFilterFeedData($aPostParams, true,$userId);
...
/*     //Function is used to show detail of feed data
    ...
    */
    ...
    public function unSaveOutfitAction() { 
        $status = array();
        $oService = $this->getServiceLocator();
        $oAuth = $oService->get('AuthService');
        $userInfo = $oAuth->getIdentity();
        if ($oAuth->hasIdentity()) {
            ...
            if ($request->isPost()) {
                ...
                $oOutfitData = $oService->get('OutfitClosesetTable');
                $oOutfitData->unSaveEntireOutfit( base64_decode($data['outfitId']), $userInfo->userId );
                $status['status'] = "Y";
