...
class MyboughtController extends BaseActionController {
    ...
    public function outfitboughtAction() {
        ...
        if ($oAuth->hasIdentity()) {
            ...
            if ($request->isPost()) {
                ...
                /*remove all article of outfit from cart*/
                    $oFeedMapping = $this->getServiceLocator()->get('OutfitsFeedsMappingTable');
                    $cartTable = $this->getServiceLocator()->get('CartTable');
                    $outfitDetails = $oFeedMapping->getOutfitsMapping(array('outfit_id' => base64_decode($data['outfitId'])), true);
                    if(count($outfitDetails) > 0) {
                        $listArticleId = array(); 
                        foreach ($outfitDetails as $value) {
                            $listArticleId[] =(int)$value['id'];
                        }
                        $where = new Where();
                        $where->in('feed_id',$listArticleId)
                              ->AND 
                              ->equalTo('user_id',$this->userId);
                        $cartTable->delete(array('where'=>$where));
                    }     
