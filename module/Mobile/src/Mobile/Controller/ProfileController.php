...
//      }
    ...
    public function sizeAction() {
        ...
        if (count($aPostParams)) {
            ...
            //$this->redirect()->toUrl('/profile/brands');
            $this->redirect()->toUrl('/profile/price');
        }

        $__viewVariables = array_merge($__viewVariables, $aStyles);
        // echo '<pre style="color:blue">';
        // var_dump( $__viewVariables);
        // echo '</pre>';    
        // die(__FILE__);
        return $__viewVariables;
    }
    ...
    public function signupAction() {
        ...
        $aPostParams = $this->params()->fromPost(); 
        if (count($aPostParams)) { //print_r($aPostParams); die;
            ...
            if( !$oUsers->isUnique('email', $aPostParams['email']) ) {
                $__viewVariables['userAlreadyExist'] = true;
            }
        }
    ...
    public function forgotpasswordAction() {
        ...
        if (count($aPostParams)) {
            ...
            //$encryptemail = base64_encode($aPostParams['email']);


            $url = 'http://' . $_SERVER['SERVER_NAME'] . '/user/changepassword/' . str_replace('/','-',$encryptemail);
            // $changepwdlink = '<a href="' . $url . '">Click Here</a>';
            // $body = 'We receive your request to change password on eLnove. To change password, please ' . $changepwdlink;
            ...
            //$this->flashMessenger()->addSuccessMessage('You will receive an email with instructions on how to reset your password in a few minutes.');
        }
    ...
    public function myaccountAction() {
        ...
        if ($oAuth->hasIdentity()) {
            ...
            if (count($aPostParams)) {
                ...
                $aPostParams['profile-pp-val'] = ($aPostParams['profile-pp-val']=='on') ? 1 : 0;
                $aPostParams['description'] = null;
